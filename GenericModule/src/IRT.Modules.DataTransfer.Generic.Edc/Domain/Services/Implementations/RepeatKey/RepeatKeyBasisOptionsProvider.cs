...
{
    public class RepeatKeyBasisOptionsProvider : IrtOptionsProvider
    {
        public override IEnumerable<ExtendedPropertyOption> GetOptions(
            EntityInfo entityInfo,
            ExtendedPropertyMetadata extendedPropertyMetadata)
        {
            var result = Enum.GetValues<RepeatKeyBasis>();

            return result
                .Select(x => new ExtendedPropertyOption
                {
                    Text = EnumHelper.GetDisplayName(typeof(RepeatKeyBasis), x) ?? x.ToString(),
