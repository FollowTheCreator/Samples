...
{
    public class ReplacementVisitsStrategyOptionsProvider : IrtOptionsProvider
    {
        public override IEnumerable<ExtendedPropertyOption> GetOptions(
            EntityInfo entityInfo,
            ExtendedPropertyMetadata extendedPropertyMetadata)
        {
            var result = Enum.GetValues<ReplacementVisitsStrategy>();

            return result
                .Select(x => new ExtendedPropertyOption
                {
                    Text = EnumHelper.GetDisplayName(typeof(ReplacementVisitsStrategy), x) ?? x.ToString(),
